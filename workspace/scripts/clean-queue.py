...
import os, re, sys, subprocess, time, urllib.request, urllib.error, ssl

QUEUE_PATH = os.path.expanduser("~/.openclaw/workspace/job-queue.md")

def check_url(url, timeout=10):
    """Returns (alive, status_code, reason)"""
    # Security: check for any existing scheme first
    if not re.search(r"^[a-z0-9+.-]+://", url, re.I):
        url = "https://" + url

    # Security: strictly validate protocol to prevent SSRF
    if not re.match(r"^https?://", url, re.I):
        return (False, 0, "Invalid protocol (must be http/https)")
